...
int execProgram(char* pgmName, char* args[])
{
  ...
  else
  {
    ...
    if (pid == 0) {
      ...
      if (retCode == -1) {
        fprintf(stderr, "My job is to stop Britain executing unknown commands. -Margaret Tatcher\n");
...
int readCommand(FILE* src, char* buffer, int size) {
  ...
  if (i >= size && feof(src) != 0) {
    fprintf(stderr, "Buffer overflows don't fall from heaven, they have to be earned here on earth. -Margaret Tatcher");
    return -2;
  }
  buffer[i] = '\0';
  return EOF;
