    ...
    while (!quit) {
        ...
        if (!quit) {
            ...
            if (args[0] != NULL) {
                ...
                fprintf(stderr, "In my lifetime all our problems have come from mainland Europe and all the solutions have come from the unexpected \"%c\" across the world. -Margaret Tatcher", flag);
            }
            else
                fprintf(stdout, "%s %% ",cwd);
